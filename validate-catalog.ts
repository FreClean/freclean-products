...
const schemaPath = resolve(__dirname, 'product.schema.json');
const catalogPath = resolve(__dirname, 'products.yaml');
...
if (hasErrors) {
  ...
  console.log(`✅ ${catalog.length} product(s) validated against product.schema.json; no invented availability claims found.`);
}
